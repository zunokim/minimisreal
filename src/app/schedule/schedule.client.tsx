...
export default function ScheduleClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const mapped: CalEvent[] = (json.events || []).map((e: any) => ({
        id: e.id,
        title: e.summary,
  ...
  // 🔒 무한 업데이트 방지
  const handleDateSet = useCallback((arg: any) => {
  ...
  const onSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasId) {
        ...
        const payload: any = { title: form.title, allDay }
        if (allDay) {
          payload.start = form.startInput
          payload.end = form.endInput || form.startInput
        } else {
          const s = new Date(form.startInput)
          const e = form.endInput ? new Date(form.endInput) : new Date(s.getTime() + 60 * 60 * 1000)
