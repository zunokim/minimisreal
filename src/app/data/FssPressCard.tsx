    ...
    <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow p-5 w-full xl:w-[720px]">
      <div className="mb-4">
        ...
        <p className="text-sm text-zinc-500 mt-1">
          금융감독원 보도자료를 기간·제목·발행기관으로 조회합니다.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {data?.result?.map((item) => (
          <article key={item.contentId} className="border rounded-xl p-4">
            ...
            {item.contentsKor && (
              <p className="mt-2 text-sm line-clamp-3 whitespace-pre-line">
                {item.contentsKor}
              </p>
            )}
        ...
        {data && data.resultCnt === 0 && !error && (
          <div className="text-sm text-zinc-500">조회 결과가 없습니다.</div>
        )}
      </div>
    </div>
  )
