  ...
  useEffect(() => {
    const loadOptions = async () => {
      ...
      for (const r of regionData ?? []) {
        regionMap.set((r as any).region_code, (r as any).region_name)
      }
      ...
      for (const r of itmData ?? []) {
        itmMap.set((r as any).itm_id, (r as any).itm_name)
      }
          ...
          <div className="fixed inset-x-0 top-16 mx-auto max-w-6xl z-50">
            <div className="rounded-2xl border bg-white shadow-xl p-4 md:p-6">
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-xs text-gray-500">{DATASETS[open].source}</div>
              ...
              <div className="mt-3 text-xs text-gray-500">
                출처: {DATASETS[open].source} · 테이블: {tableName}
              </div>
            </div>
          </div>
        </>
