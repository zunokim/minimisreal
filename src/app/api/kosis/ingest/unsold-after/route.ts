// src/app/api/kosis/ingest/unsold-after/route.ts
import { NextResponse, type NextRequest } from 'next/server'

type InBody = { start?: string; end?: string; region?: string }
...
export async function POST(req: NextRequest) {
  try {
    ...
    url.searchParams.set('dataset', 'unsold_after')
    url.searchParams.set('orgId', '116')
    url.searchParams.set('tblId', 'DT_MLTM_5328')
    ...
    if (body.region) url.searchParams.set('objL1', body.region)
